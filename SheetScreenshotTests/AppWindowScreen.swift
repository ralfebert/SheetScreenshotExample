...
class AppWindowScreen<T: View> {
    var controller: UIHostingController<T>!
    var dispose: (() -> Void)?
    let view: () -> T
    let window = UIApplication.shared.keyWindow!

    init(@ViewBuilder view: @escaping @MainActor  () -> T) async {
    ...
    func renderImage() -> UIImage {
        self.window.asImage(drawHierarchy: true)
    }
    ...
    func assertScreenshot(named name: String? = nil, file: StaticString = #file, testName: String = #function, line: UInt = #line) async {
        precondition(Thread.isMainThread)
        let image = await self.updateAndRenderImage()
        assertSnapshot(of: image, as: .image(perceptualPrecision: 0.95), named: name, file: file, testName: testName, line: line)
    }
    ...
    @MainActor deinit {
        if self.controller != nil {
            fatalError("Call tearDown explicitly to make sure it's gone in the test method")
        }
    }
}
